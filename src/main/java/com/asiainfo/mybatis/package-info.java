 ...
 * - 如果拿它跟具有相同功能的 JDBC 代码进行对比，你会立即发现省掉了将近 95% 的代码。MyBatis 为聚焦于 SQL 而构建，以尽可能地为你减少麻烦。
 ...
 * - select
   ...
   - #{id} 这就告诉 MyBatis 创建一个预处理语句（PreparedStatement）参数, PreparedStatement ps = conn.prepareStatement(selectById); ps.setInt(1,id);
 ...
 * - insert, update 和 delete
   - 插入语句的配置规则更加丰富，在插入语句里面有一些额外的属性和子元素用来处理主键的生成，而且有多种生成方式。
    <insert / update / delete
      ...
      keyColumn=""                        - （仅对 insert 和 update 有用）通过生成的键值设置表中的列名，这个设置仅在某些数据库（ PostgreSQL）是必须的，
   ...
   - 如果你的数据库支持自动生成主键的字段（比如 MySQL 和 SQL Server），那么你可以设置 useGeneratedKeys=”true”，然后再用keyProperty="id" 设置到目标属性上就 OK 了。
 ...
 * - selectKey
    <selectKey                  - selectKey 语句结果应该被设置的目标属性。如果希望得到多个生成的列，也可以是逗号分隔的属性名称列表。
      ...
      resultType="int"          - 结果的类型。MyBatis 通常可以推断出来，但是为了更加精确，写上也不会有什么问题。MyBatis 允许将任何简单类型用作主键的类型，包括字符串。
 ...
 * - 参数#{}
   - 参数是 MyBatis 非常强大的元素。大约 90% 的情况下你使用的都是简单参数。
   - 原始类型或简单数据类型（比如 Integer 和 String）因为没有相关属性，它会完全用参数值来替代。
   ...
   - javaType 几乎总是可以根据参数对象的类型确定下来，除非该对象是一个 HashMap。这个时候，你需要显式指定 javaType 来确保正确的类型处理器（TypeHandler）被使用。
 ...
 * - 字符串替换
   ...
   - 不过有时你就是想直接在 SQL 语句中插入一个不转义的字符串， ${}字符串替换将会非常有用。
 ...
 * - resultMap
   ...
   - 如果列名和属性名没有精确匹配，可以在 SELECT 语句中对列使用别名（这是一个基本的 SQL 特性）来匹配标签。
   - 也可以使用外部的 resultMap ，这也是解决列名不匹配的另外一种方式。
   ...
   - resultMap
    <resultMap 
      ...
      <association property="author" javaType="Author">     - 一个复杂类型的关联；许多结果将包装成这种类型，关联本身可以是一个 resultMap 元素，或者从别处引用
      ...
      <collection property="posts" ofType="Post">           - 一个复杂类型的集合， 集合本身可以是一个 resultMap 元素，或者从别处引用
        ...
        <discriminator javaType="int" column="draft">       - 使用结果值来决定使用哪个 resultMap
          <case value="1" resultType="DraftPost"/>          - case 基于某些值的结果映射，case 本身可以是一个 resultMap 元素，因此可以具有相同的结构和元素，或者从别处引用
        </discriminator>
      </collection>
    </resultMap>
   ...
   - 鉴别器(discriminator)
   - 有时候，一个数据库查询可能会返回多个不同的结果集（但总体上还是有一定的联系的）。 鉴别器就是被设计来应对这种情况的，它很像 Java 语言中的 switch 语句。
 ...
 * - 甚至在提供了结果映射后，自动映射也能工作。在这种情况下，对于每一个结果映射，在 ResultSet 出现的列，
 * - 如果没有设置手动映射，将被自动映射。在自动映射处理完毕后，再处理手动映射。 
 * 
 * 
 * - 缓存
 ...
 * - 它也允许你指定开头与结尾的字符串以及在迭代结果之间放置分隔符。通常是在构建 IN 条件语句的时候。
 ...
 * @Param
 * @SelectKey               ->  <selectKey>
 * @InsertProvider @UpdateProvider @DeleteProvider @SelectProvider 动态sql支持
 * 
 * @author chenzq  
 * @date 2019年5月3日 下午1:25:00
 * @version V1.0
